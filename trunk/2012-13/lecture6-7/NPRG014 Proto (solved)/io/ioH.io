...
Class withConstructor := method(
	...
	self new := block(
		this := __instanceProto shallowCopy
		this setProto(cls)
...
Class withInstanceAttribute := method(name, val,
	__instanceProto setSlot(name, val)
)

Class withStaticAttribute := method(name, val,
...
Class withMethod := method(
	...
	mtd setMessage(bodyMsg)
	mtd setScope(nil)
	
	self setSlot(methodName, mtd) 
	mtd setIsActivatable(true)
)
// ----------------------
